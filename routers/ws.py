...
async def ws_chat(websocket: WebSocket):
    ...
    try:
                ...
                try:
                    with get_db() as conn:
                        with conn.cursor() as cursor:
                            cursor.execute(
                                "SELECT lang, user_id FROM sessions WHERE session_id = %s AND is_active = 1",
                                (session_id,),
                            )
                            row = cursor.fetchone()
                        if not row:
        ...
        logger.info(f"[WS] session disconnected (user={user_id})")
    except Exception as exc:
...
async def ws_chat_session(websocket: WebSocket, session_id: str):
    """
    Session-scoped WebSocket chat endpoint.
    Session ownership is verified once at connect time.
    ...
    # ── Verify session ownership once at connect ──────────────────────────
    sess_lang = "en"
    try:
        with get_db() as conn:
            with conn.cursor() as cursor:
    ...
    await websocket.accept()
    logger.info(f"[WS] session {session_id!r} accepted for user={user_id}")
